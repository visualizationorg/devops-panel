    ...
    try {
        ...
        do {
            const wiqlResponse = await axios.post(
                `https://dev.azure.com/${getOrganization()}/${projectName}/_apis/wit/wiql?api-version=7.2-preview.2`,
...
export const fetchWorkItemDetails = async (ids, fields = 'System.TeamProject,System.IterationPath,System.IterationLevel1,System.IterationLevel2,System.State,System.AssignedTo') => {
    try {
        const response = await axios.get(
...
export const fetchPullRequests = async (projectName, repositoryId) => {
    try {
        const response = await axios.get(
            `https://dev.azure.com/${getOrganization()}/${projectName}/_apis/git/repositories/${repositoryId}/pullrequests?api-version=7.0`,
            { headers: getBaseHeaders() }
        );
        return response.data.value;
    } catch (error) {
